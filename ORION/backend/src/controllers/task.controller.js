...
const updateTask = async (req, res) => {
    try {
        const { id } = req.params;
        const { id: taskId, user_id, ...camposAActualizar } = req.body;
        ...
        return res.status(200).json({ updateTask: updatedTaskData });
    } catch (err) {
        return res.status(500).json({ message: 'Error en el servidor', error: err.message });
    }
};
...
const deleteTask = async (req, res) => {
    try {
        ...
        const { data: deletedTaskData, error } = await supabase
            .from('task')
            .delete()
            .eq('id', id)
            .select()
        ...
        return res.status(200).json({ deleteTask: deletedTaskData });
    } catch (err) {
        return res.status(500).json({ message: 'Error en el servidor', error: err.message });
    }
};
